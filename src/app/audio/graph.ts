...
export interface AudioGraphDelegate<Node extends BaseNode> {
...
export class AudioGraph<Node extends BaseNode> {
  constructor(private delegate: AudioGraphDelegate<Node>) {}
  ...
  private buildAudioGraph({
    ...
    if (addNode) {
      ...
      newNode = produce(newNode, (node) => {
        ...
        node.backingNode = this.delegate.createNode(newNode.type);
      });
      if (parent) {
  ...
  private setupAuxConnections(node: Node) {
    if (node.auxConnections) {
      node.auxConnections.forEach((key) => {
        ...
        if (auxNode) {
          this.delegate.connectNodes(node.backingNode, auxNode?.backingNode);
        } else {
          throw new Error(
